...
def unique_username(requested, conn, exclude_member_id=None):
    requested = clean_username(requested)
    base = requested or "member"
    if not re.fullmatch(r"[a-z0-9_-]{3,40}", base):
        base = re.sub(r"[^a-z0-9_-]+", "_", base)[:36].strip("_") or "member"
    candidate = base[:40]
    n = 2
    while True:
        app_row = conn.execute("SELECT id FROM applications WHERE username=?", (candidate,)).fetchone()
        ...
        if not app_row and not member_row:
            return candidate
        suffix = f"_{n}"
...
def make_username(name, conn):
    ...
    return unique_username(base, conn)
...
def init_db():
    with db() as conn:
        ...
        for row in accepted:
            ...
            if not existing:
                username = unique_username(row["username"] or row["name"], conn)
                conn.execute("INSERT INTO members(name,username,github,skills,role,bio,active,application_id) VALUES(?,?,?,?,?,?,1,?)", (row["name"], username, row["github"] or "", row["skills"], "NEXORA Member", "", row["id"]))
        conn.commit()
...
def create_application():
    ...
    with db() as conn:
        username = unique_username(username, conn)
        status_token = make_status_token()
        cur = conn.execute("INSERT INTO applications(name,username,github,skills,why,score,status_token) VALUES(?,?,?,?,?,?,?)", (name, username, github, skills, why, score, status_token))
        conn.commit()
...
def application_status(status_token):
    with db() as conn:
        row = conn.execute("SELECT name,username,score,status,created_at FROM applications WHERE status_token=?", (status_token,)).fetchone()
    if not row:
    ...
    with db() as conn:
        ...
        if status == "accepted":
            ...
            if member:
                ...
                username = unique_username(row["username"] or row["name"], conn)
                conn.execute("INSERT INTO members(name,username,github,skills,role,bio,active,application_id) VALUES(?,?,?,?,?,?,1,?)", (row["name"], username, row["github"] or "", row["skills"], "NEXORA Member", "", application_id))
...
def update_member(member_id):
    ...
    with db() as conn:
        ...
        username = clean_username(data.get("username", row["username"]))
        github = str(data.get("github", row["github"] or "")).strip()
        skills = str(data.get("skills", row["skills"])).strip()
        role = str(data.get("role", row["role"])).strip()
        bio = str(data.get("bio", row["bio"] or "")).strip()
        active = 1 if str(data.get("active", row["active"])).lower() in {"1", "true", "yes", "on"} else 0
        if not name or not skills or len(name)>100 or len(username)>40 or len(github)>200 or len(skills)>500 or len(role)>100 or len(bio)>1000:
            return jsonify({"ok": False, "error": "Invalid member data."}), 400
        if not re.fullmatch(r"[a-z0-9_-]{3,40}", username):
            return jsonify({"ok": False, "error": "Invalid username."}), 400
        username = unique_username(username, conn, member_id)
        conn.execute("UPDATE members SET name=?,username=?,github=?,skills=?,role=?,bio=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?", (name, username, github, skills, role or "NEXORA Member", bio, active, member_id))
        conn.commit()
    return jsonify({"ok": True})
...
def update_site_config():
    ...
    if len(clean.get("site_title", "")) > 160 or len(clean.get("maintenance_message", "")) > 1000:
        return jsonify({"ok": False, "error": "Setting is too long."}), 400
    with db() as conn:
