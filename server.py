...
def create_application():
    ...
    if len(name) > 100 or len(username) > 30 or len(github) > 200 or len(skills) > 300 or len(why) > 3000:
        return jsonify({"ok": False, "error": "Application is too long."}), 400
    if not all(ch.isalnum() or ch in "._-" for ch in username):
        return jsonify({"ok": False, "error": "Username can use letters, numbers, dot, underscore and hyphen only."}), 400
...
def application_status(status_token):
    with db() as conn:
        row = conn.execute(
            "SELECT name,username,score,status,created_at FROM applications WHERE status_token=?",
            (status_token,),
        ).fetchone()
    if not row:
...
def admin_applications():
    with db() as conn:
        rows = conn.execute(
            "SELECT id,name,username,github,skills,why,score,status,created_at FROM applications ORDER BY id DESC"
        ).fetchall()
